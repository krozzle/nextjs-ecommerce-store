...
export default function ShoppingCart(cart, products) {
  ...
  // todo function handleUnits(e) {
  // todo e.target.value * props.cart.price;
  // todo }

  // define a single value for a single unit
  const unitPrice =
    cart === undefined
      ? []
      : cart.map(unit => {
  ...
  // add all the single values of each unitPrice into a single total value
  const total =
    unitPrice === undefined
      ? 0
      : unitPrice.reduce((acc, priceForEach) => {
          return acc + priceForEach;
        }, 0);
